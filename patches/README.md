...
|---|---|---|
| **`mqtt-client`** | `/usr/local/lib/eatabit/bin/mqtt-client.js`, `/etc/systemd/system/mqtt-client.service` | 1. [`2026-05-12-watchdog-exit-hang`](./2026-05-12-watchdog-exit-hang/)<br>2. [`2026-06-30-offline-reboot-and-expired-job`](./2026-06-30-offline-reboot-and-expired-job/)<br>3. [`2026-08-20-ngrok-session-reclaim`](./2026-08-20-ngrok-session-reclaim/) — **self-contained**<br>4. [`2026-08-19-mqtt-keepalive-tolerance`](./2026-08-19-mqtt-keepalive-tolerance/) — **requires 3**<br>5. [`2026-08-23-app-permissions-and-shadow-churn`](./2026-08-23-app-permissions-and-shadow-churn/) — **requires 4**<br>6. [`2026-08-24-mqtt-never-connected-watchdog`](./2026-08-24-mqtt-never-connected-watchdog/) — **requires 5**; ⛔ **BENCH ONLY** |
...
| 2026-08-24 | [`2026-08-24-mqtt-never-connected-watchdog`](./2026-08-24-mqtt-never-connected-watchdog/) | devices at the `2026-08-23-app-permissions-and-shadow-churn` end state (sha `7ecbf0ea…`), which is also repo source on both lines — checksum-gated, **js only**, exactly one accepted prior | **High (P1)** — after a connection-watchdog restart the process retries a poisoned CRT host resolver every 30 s **forever**: the `ClientBootstrap` that owns the resolver is never rebuilt, and the Layer 1 watchdog cannot fire in a process that has never connected, while `SdNotify.watchdog()` is fed unconditionally so systemd sees a healthy unit. `00000000c3343f47`: **13,840** consecutive `AWS_IO_DNS_QUERY_FAILED`, longest run **4,919 over 41 h**, **115.3 h offline across 7 outages (28% of 417 h)**, every recovery a power cycle; `0000000042288e07` independently shows 5,131 failures against 91 watchdog fires. Arms the watchdog on a separate `neverConnectedSince` clock **bounded at 3 restarts** (counter in `/run/eatabit/`, preserving v1.0.10's no-reboot-loop guarantee for a genuinely offline device) and rebuilds the bootstrap per attempt. Recovery is the **restart**, not a reboot — `StartLimitBurst=5` is arithmetically unreachable at ~193 s/cycle. ⛔ **BENCH ONLY — not authorized for any field device**, `00000000c3343f47` included. (`BUG-057`) |
