...
- Wi-Fi power-save is now explicitly **off**. No image set it, so every gateway ran with the `brcmfmac` driver default — on — which delays traffic into the device until the next beacon and is a known source of dropped associations on the Pi Zero 2 W. The image now installs `/etc/NetworkManager/conf.d/eatabit-wifi-powersave.conf` (`[connection] wifi.powersave = 2`) from `stage2/02-net-tweaks/`, a global default that NetworkManager re-applies at every activation, so it also covers networks provisioned later from the app. This is hardening, not a diagnosed fix: power-save is not proven to have caused any specific outage (`BUG-093` → *Honest scope*). Devices already in the field get the same file from the field patch `patches/2026-09-23-wifi-powersave-off/`, which restarts nothing and drops no connection. v1.0.11 does **not** carry this; it ships in the next release. (`BUG-093`)
- A device that loses its network now recovers without a power-cycle (`BUG-094`). Every existing recovery layer watched the MQTT session or the Node process and none watched the link, so a wedged radio, driver or NetworkManager state stayed offline indefinitely. A new `netwatch` timer (`stage3/14-install-netwatch/`), independent of `mqtt-client`, escalates by accumulated offline time — snapshot at 2 min, `nmcli` reconnect at 3, radio cycle at 6, `brcmfmac` driver reload at 7, reboot at 30, the ladder starting over on every boot — and stops as soon as the client or its own DNS + TCP probe of the IoT endpoint succeeds. It acts only on a network that has worked (NetworkManager connection UUID + SSID), never during a print, and never reboots in the first 5 minutes after boot; after a reboot it causes, neither the booting nor the ready receipt prints, while a normal boot or human power-cycle still prints both. Every step is logged to the card-backed `netwatch.log`; on recovery the device publishes `device.networkRecovered` and the health shadow carries `lastNetworkRecovery`. The health shadow also gains `bssid`, and its RSSI now comes from the live association (`iw dev wlan0 link`) instead of the last scan result. Field patch: `patches/2026-09-23-netwatch`.
- Application logs actually rotate now. v1.0.11's fix for rotation (the 0755 log directory and the `eatabit-*` logrotate stanzas) was correct but never once ran: Debian's stock `logrotate.service` has `ProtectSystem=full`, which makes `/usr` read-only inside that unit, and the eatabit logs live under `/usr/local/lib/eatabit/log`, so every automatic run failed with `Read-only file system`, exited 1 — failing the whole nightly run, so nothing on the device rotated — and still advanced its state file. The one rotation ever recorded was a manual `logrotate -f` from a shell, which runs outside the sandbox. Observed on at least 25 devices across both hardware lines. The image now installs `/etc/systemd/system/logrotate.service.d/eatabit.conf` (`ReadWritePaths=-/usr/local/lib/eatabit/log`) from `stage3/05-install-log2ram/`, the same grant the eatabit units already carry in their own unit files; `ProtectSystem=full` itself is kept, so the rest of `/usr` stays read-only to logrotate. No logrotate stanza changes. Devices in the field get the same file from `patches/2026-09-26-logrotate-log-dir-write/`, which restarts nothing. The first run after it lands rotates months of accumulated `mqtt-client.log` (up to ~14 MB observed), and `rotate 7` then begins discarding old generations. (`BUG-097`)
