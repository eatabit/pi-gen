...
- `mqtt-client` no longer stalls permanently after a connection-watchdog restart. Two defects in `stage3/03-install-mqtt-client/files/mqtt-client.js` combined to strand a device indefinitely: the Layer 1 watchdog was gated on `lastDisconnectedAt`, which is assigned only by the `interrupt`/`disconnect` handlers and so is never set in a process that has **never** connected — leaving the watchdog unarmed in exactly that state while `SdNotify.watchdog()` was fed unconditionally, so systemd saw a healthy process. Meanwhile the `ClientBootstrap`, which owns the CRT host resolver, was built once per process and reused across every initial-connect retry, so once its resolver held a failed entry for the IoT endpoint each retry inherited that failure. One field device logged **13,840** consecutive `AWS_IO_DNS_QUERY_FAILED` retries — the longest run 4,919 over **41 hours**, with the site's network up throughout — and every one of 7 recoveries required a power cycle. The watchdog now also arms on a separate `neverConnectedSince` clock (`lastDisconnectedAt` is deliberately *not* seeded, so health telemetry still reports `null` for a process that never connected), and the bootstrap, client and config are rebuilt for every attempt. The never-connected exit is **bounded at 3 restarts** via a counter in `/run/eatabit/`: enough to clear a process-local wedge, after which the process stays up and retries quietly so a genuinely offline or unprovisioned device is not reboot-looped — preserving the 1.1.4 behaviour. Recovery is the restart, not a reboot; `StartLimitAction=reboot-force` is arithmetically unreachable on this path and is not relied on.
