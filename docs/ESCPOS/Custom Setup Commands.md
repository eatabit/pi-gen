...
1b 1c 26 20 56 31 20 73 65 74 6b 65 79 0d 0a 00 db 00 04 00 01 01 01               ← setkey 0xDB
...
|-----|-------|-------------|
| `0x93` | `01 01 01` | Disable human voice, enable buzzer (zone `0x00`, key `0x93`) |
| `0xDB` | `00 04 00 01 01 01` | Built-in buzzer config (zone `0x00`, key `0xDB`, 4 data bytes) |

**0xDB data bytes:**

| Byte | Setting | Value | Range |
|------|---------|-------|-------|
| 1 | Sound after cutting | `00` (OFF) | `00`=off, `01`=on |
| 2 | Number of beeps | `01` | 1–255 |
| 3 | Beep duration | `01` (100ms) | 1–64 (×100ms) |
| 4 | Beep interval | `01` (100ms) | 1–64 (×100ms) |

> **Note:** "Sound after cutting" must be OFF (`00`) so the printer's optical paper sensor can drive continuous buzzing until paper is pulled. If ON, each cut triggers a one-shot beep that resets the sensor-driven buzzer cycle.
