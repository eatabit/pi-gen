...
# nmcli reconnect, a radio cycle, a driver reload and finally a reboot at 30 min
# when the device has lost the network -- starting the ladder over on every boot --
# and logs every step to a card-backed log. See files/netwatch.js for the ladder
# and its guards.
# ------------------------------------------------------------------------------
...
# Card-backed state: the outage's reporting totals, the network fingerprint and the
# receipt-suppression marker must all survive the reboot they describe -- so NOT tmpfs.
